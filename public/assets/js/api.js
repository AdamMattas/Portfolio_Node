...
$(document).on("ready", function() {
  ...
  function getYouTube(search) {
    ...
    var queryURL =
      ...
      search +
      "&type=video&key=AIzaSyD_s7PmE89zIcTXeTYWnClw0uOYh8446NU";
        ...
        if (typeof NYTData.response.docs[i].multimedia[1] != "undefined") {
          ...
          wellBackground.attr(
            ...
            "https://static01.nyt.com/" +
              NYTData.response.docs[i].multimedia[1].url
          );
        } else {
          var wellBackground = $("<img>");
          wellBackground.addClass("article-img");
  ...
  $(document).on("click", "#tmdb-search", function() {
    ...
    function getCrewDetails(id) {
      var queryCrewDetails =
        "https://api.themoviedb.org/3/movie/" + id + "/credits?api_key=" + key;

      $.ajax({ url: queryCrewDetails, method: "GET" }).done(function(
        responseCrew
      ) {
