...
endeavor = {
    ...
    body:'At its core, Endeavor is a site for posting jobs. However, Endeavor has many features that make it a very useful tool. For starters, when a user posts a new job, they can specify it as "Accepting Bids" and define a target dollar amount. This allows other users that are interested in completing the job to submit a bid for consideration. The submitted bid can be accepted or rejected by the job poster. Similar to the bidding style of posting, a user can also post a job at a fixed price.'<br />'Every user is able to customize their profile page with images, a biography, and professional resume. The owner of the profile page can easily update the information displayed on their profile from the preferences tab while logged in. In addition to information that the account owner adds, there is also a star rating display. The star rating is calculated based on feedback received from Endeavor users that have hired and worked with the account owner. Individual performance reviews are located directly below the star rating with a link to view details about the completed job.'<br />'Endeavor also has a notification system to inform users when there is activity relating to their account. Notifications are displayed via a news style ticker visible at the top of the screen. Clicking on the ticker will navigate to a page to manage new notifications and reply to any private messages sent using our messaging system.'<br />'Endeavor is powered by Node, Express, Passport, Handlebars, jQuery, MySQL, ORM for database queries, and an MVC file structure.',
